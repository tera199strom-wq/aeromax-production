...
const galleryImages = [
  { img: img1, rotateLeft: false },
  { img: img2, rotateLeft: false },
  { img: img3, rotateLeft: false },
  { img: img4, rotateLeft: false },
  { img: img5, rotateLeft: false },
  { img: img6, rotateLeft: true },
];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {galleryImages.map((item, index) => (
            <div 
              key={index} 
              className="group relative aspect-square overflow-hidden rounded-2xl bg-slate-200 shadow-lg transition-transform duration-500 hover:-translate-y-2"
            >
              <HeicImage 
                src={item.img} 
                alt={`Gallery image ${index + 1}`} 
                className={`w-full h-full object-cover transition-transform duration-700 ${item.rotateLeft ? 'rotate-left-square' : 'group-hover:scale-110'}`}
