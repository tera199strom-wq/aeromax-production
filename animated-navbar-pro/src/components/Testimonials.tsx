  ...
  return (
    <div className="flex overflow-hidden select-none gap-6 group">
      <div className={`flex flex-shrink-0 gap-6 min-w-full ${direction === 'left' ? 'animate-marquee' : 'animate-marquee-reverse'} group-hover:pause`}>
        {items.map((item, i) => (
          <div key={i} className="flex-shrink-0 w-[280px] sm:w-[350px] p-6 sm:p-8 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            <p className="text-slate-600 italic mb-6 text-sm sm:text-base">"{item.text}"</p>
            <div>
              <h4 className="font-bold text-slate-900 text-sm sm:text-base">{item.name}</h4>
              <p className="text-xs sm:text-sm text-[#8b0000] font-medium">{item.role}</p>
            </div>
          </div>
        ))}
      </div>
